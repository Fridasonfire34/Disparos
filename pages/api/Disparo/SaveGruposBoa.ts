  ...
  try {
    ...
    for (const grupo of grupos) {
      ...
      const checkResult = await pool
        ...
        .query(
          'SELECT [Grupo Logistico], [Color Grupo] FROM [Grupos BOA] WHERE [Grupo Logistico] = @GrupoLogistico'
        );
      ...
      await pool
        ...
        .query(
          'INSERT INTO [Grupos BOA] ([Grupo Logistico], [Color Grupo], [ID]) VALUES (@GrupoLogistico, @ColorGrupo, @ID)'
        );
    ...
    console.error('Error saving Grupos BOA:', error);
    if (pool) {
      try {
        await pool.close();
      } catch (closeError) {
    ...
    return res.status(500).json({ error: 'Error al guardar Grupos BOA' });
  }
}
