...
export default async function handler(
  ...
  try {
    ...
    const result = await pool
      ...
      .query('DELETE FROM [Grupos BOA] WHERE [Grupo Logistico] = @GrupoLogistico');
    ...
    console.error('Error deleting Grupos Boa:', error);
    if (pool) {
      try {
        await pool.close();
      } catch (closeError) {
    ...
    return res.status(500).json({ error: 'Error al eliminar Grupo Boa' });
  }
}
